...
class ChartMaker:
    ...
    def make_course_chart(courses: list[int], offer_course: int) -> None:
        ...
        plt.axhline(y=median_course, color='r', linestyle='-', label=f'Avg course = {median_course:.0f} KM')
        plt.axhline(y=offer_course, color='y', linestyle='--', label=f'This offer course = {offer_course:.0f} KM')
